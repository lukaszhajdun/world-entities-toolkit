  ...
  return Promise.all(
    keyFigures.map(async (entry, index) => {
      ...
      return {
        ...
        ...createActorReferencePresentation(reference, resolved, "WET.Faction.KeyFigures.UnknownName")
      };
    })
  );
}
...
export function prepareFactionHavens(actor) {
  return prepareFactionReferenceCollection(actor, "havens", "WET.Faction.Havens.UnknownName");
}

export function prepareFactionChallenges(actor) {
  return prepareFactionReferenceCollection(actor, "challenges", "WET.Faction.Challenges.UnknownName");
}
