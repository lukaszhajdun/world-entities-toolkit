...
export function getStorageSlotMetadata(actor, slotId) {
  ...
  return {
    ...
    canEdit,
    canModifyItems,
    hostName: actor.name ?? game.i18n.localize("WET.Sheets.Common.Unnamed")
  };
}
...
export function prepareStorageItems(actor, slotId) {
  ...
  return getStorageItems(actor, slotId).map(item => ({
    id: item.id,
    uuid: item.uuid,
    name: item.name ?? game.i18n.localize("WET.Storage.Items.Unnamed"),
...
export function prepareStorageWindowContext(actor, slotId) {
  ...
  return {
    ...
    storage: {
      ...
      emptyLabel: game.i18n.localize("WET.Storage.Empty.Items")
    },
    items
  };
}
