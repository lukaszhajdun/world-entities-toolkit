...
export class FactionActorSheet extends BaseModuleActorSheet {
  ...
  async _onDropActor(event, actor) {
    ...
    if (!this.canEditDocument) {
      ui.notifications?.warn(game.i18n.localize("WET.Faction.Notifications.DropLocked"));
      return null;
    }
    ...
    switch (result?.status) {
      case "added":
        ui.notifications?.info(game.i18n.localize("WET.Faction.Notifications.Added"));
        return actor;
      case "duplicate":
        ui.notifications?.warn(game.i18n.localize("WET.Faction.Notifications.AlreadyAdded"));
        return null;
      case "self":
        ui.notifications?.warn(game.i18n.localize("WET.Faction.Notifications.CannotAddSelf"));
        return null;
      default:
        ui.notifications?.warn(game.i18n.localize("WET.Faction.Notifications.InvalidDrop"));
        return null;
    }
  }
  ...
  async _onKeyFigureOpen(button) {
    ...
    if (!opened) {
      ui.notifications?.warn(game.i18n.localize("WET.Faction.Notifications.MissingActor"));
    }
  }
...
export function registerFactionActorSheet() {
  ...
  foundry.documents.collections.Actors.registerSheet(MODULE_ID, FactionActorSheet, {
    types: [FACTION_TYPE],
    makeDefault: true,
    label: game.i18n.localize("WET.Sheets.Faction.Label")
  });
