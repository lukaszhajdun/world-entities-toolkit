...
export class StorageWindow extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  get title() {
    if (!this.actor) return game.i18n.localize("WET.Storage.Window.TitleFallback");

    const context = prepareStorageWindowContext(this.actor, this.slotId);
    return game.i18n.format("WET.Storage.Window.Title", {
  ...
  async _onDropToStorage(event) {
    ...
    if (!dragData) {
      ui.notifications?.warn(game.i18n.localize("WET.Storage.Notifications.InvalidDrop"));
      return;
    }
    ...
    switch (result.status) {
      ...
      case "moved":
        await this.#queueRefresh();
        ui.notifications?.info(game.i18n.localize("WET.Storage.Notifications.ItemMovedToStorage"));
        return;

      case "copied":
        await this.#queueRefresh();
        ui.notifications?.info(game.i18n.localize("WET.Storage.Notifications.ItemCopiedToStorage"));
        return;

      case "alreadyStored":
        await this.#queueRefresh();
        ui.notifications?.info(game.i18n.localize("WET.Storage.Notifications.ItemAlreadyInStorage"));
        return;

      case "disabled":
        ui.notifications?.warn(game.i18n.localize("WET.Storage.Notifications.StorageDisabled"));
        return;
  ...
  async _onItemOpen(event, opener) {
    ...
    if (!item) {
      ui.notifications?.warn(game.i18n.localize("WET.Storage.Notifications.MissingItem"));
    }
  }
  ...
  async _onItemRemove(event, removeButton) {
    ...
    switch (result.status) {
      case "removed":
        await this.#queueRefresh();
        ui.notifications?.info(game.i18n.localize("WET.Storage.Notifications.ItemRemoved"));
        return;
  ...
  async _onCapacityChange(event, capacityInput) {
    ...
    switch (result.status) {
      ...
      default:
        ui.notifications?.warn(game.i18n.localize("WET.Errors.AutoSaveFailed"));
    }
  }
